...
Save/load
```{r}
#saveRDS(yuan, here("data", "analytic", "yuan", paste0("yuan_raw_rge_", Sys.Date(), ".rda")))
rge <- readRDS(here("data", "analytic", "yuan", "yuan_raw_rge_2023-11-09.rda"))
pd <- pData(rge) %>% as.data.frame()
clin.covariates <- c("Sex", "CellType", "Case_ID", "Study", "Sentrix_ID", "Sentrix_Position")
```

```{r, include = F, eval = F}
#saveRDS(rge, here("data", "analytic", paste0("rge_kc_vy_salas2018_", Sys.Date(), ".rda")))
# The adult blood and two placenta datasets merged (EPIC)
rge <- readRDS(here("data", "analytic", paste0("raw_rge_Campbell_Salas_Yuan_2022-11-01.rda")))
pd <- pData(rge) %>% as.data.frame()
#saveRDS(pd,file = here("data", paste0("rge_kc_vy_salas2018_2022-09-26.rda")))
```

```{r}
pd %>% filter(CellType != "Villi") %>% dim()
```
...
27 of 99 are outliers but no disagreements?
```{r}
#snps <- getSnpInfo(grs)
...
The 8 site with missing beadmean
```{r}
names(ms.noob[na.idx, ])
manifest %>% filter(probe_id %in% names(ms.noob[na.idx,]))
```

Drop the 18 probes with low bead count or NA, leaving 866073
```{r}
...
29,747 probes have average detP > 0.01
```{r}
table(detprobe$pmean > 0.01)
```

Only 3.4% are over the threshold
```{r}
...
31,375 probes had greater than 5% of samples failure rate for detP
```{r}
...
93564 cross-reactive probes dropped
```{r}
...
Save QC'd MethylSet
```{r}
#saveRDS(ms.noob.nbead.detP.xreac.auto, here("data", "analytic", "yuan", "qced_MethylSet_yuan_2023-11-10.rds"))
```
...
BMIQ norm
```{r}
beta.bmiq <- champ.norm(beta, resultsDir = here("data", "analytic", "yuan", "epic"), plotBMIQ = T, arraytype = "EPIC", cores = 8, method = "BMIQ")
...
## Save result
725512 sites across 99 samples
```{r}
dim(beta.bmiq)

#saveRDS(beta.bmiq, file = here("data", "analytic", "yuan", paste0("beta_bmiq_qced_yuan_", Sys.Date(), ".rda")))

#saveRDS(pd, file = here("data", "analytic", "yuan", paste0("beta_bmiq_qced_yuan_pd_", Sys.Date(), ".rda")))

#readRDS(here("data", "analytic", "yuan", "beta_bmiq_qced_yuan_2023-11-14.rda"))
#readRDS(here("data", "analytic", "yuan", "beta_bmiq_qced_yuan_pd_2023-11-14.rda"))

```

```{r}
rownames(plCellCpGsThird) %in% rownames(readRDS(here("data", "analytic", "yuan", "beta_bmiq_qced_yuan_2023-11-14.rda")))
```


```{r}
#saveRDS(ms.noob.nbead.detP.xreac.auto %>% getBeta(), file = here("data", "analytic", paste0("qc_beta_all_", Sys.Date(), ".rda")))
```