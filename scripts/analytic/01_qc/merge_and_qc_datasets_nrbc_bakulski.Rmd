...
# Get Bakulski and deGoede indices and subset accordingly
study.idx <- cord.pd$Study %in% c("Bakulski")
...
9 of 80 are outliers but no disagreements
```{r}
#snps <- getSnpInfo(grs)
...
24015 probes have average detP > 0.01 
```{r}
table(detprobe$pmean > 0.01)
```

5.3% are over the threshold
```{r}
...
10,400 probes had greater than 5% of samples failure rate for detP
```{r}
...
53565 cross-reactive probes dropped
```{r}
...
```{r, eval = F}
champ.QC(beta=beta, pheno=pd$Study, resultsDir = here("results", "nrbc_bmiq_bakulski"), dendrogram = T, mdsPlot = T)
```

```{r}
beta.bmiq <- champ.norm(beta, resultsDir = here("results", "nrbc_bmiq_bakulski"), plotBMIQ = T, arraytype = "450K", cores = 8)
...
#saveRDS(nrbc.pd, file = here("data", "analytic", "nrbc", "bakulski_nrbc_pd_2023-11-16.rda"))
```

```{r}
nrbc
#saveRDS(nrbc %>% getBeta(), file = here("data", "cord", "beta_noob_nrbc_qc_w_Bakulski_deGoede_2022-10-20.rda"))
```

## Save result
380319 sites for analysis across 4 samples
```{r}
beta.bmiq.nrbc <- beta.bmiq[, nrbc.idx]
dim(beta.bmiq.nrbc)
#saveRDS(beta.bmiq.nrbc, file = here("data", "analytic", "nrbc", "beta_noob_bmiq_nrbc_qc_Bakulski_2023-11-15.rda"))
```
